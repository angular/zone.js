...
var BindingMap = require('./binding-map');

function bindArguments(args, skipEnqueue) {
  for (var i = args.length - 1; i >= 0; i--) {
    if (typeof args[i] === 'function') {
      args[i] = global.zone.bind(args[i], skipEnqueue);
    }
  }
  return args;
};
...
function patchPrototype(obj, fnNames) {
  patchObject(obj, fnNames);
};

function patchObject (obj, fnNames, options) {
  options = options || {};
  options.useBindOnce = options.useBindOnce || false;
  options.skipEnqueue = options.skipEnqueue || false;

  var bindArgs;

  if (options.useBindOnce) {
    bindArgs = bindArgumentsOnce;
  } else {
    bindArgs = function (args) {
      return bindArguments(args, options.skipEnqueue);
    }
  }
  ...
  fnNames.forEach(function (name) {
    ...
    if (delegate) {
      obj[name] = function () {
        return delegate.apply(this, bindArgs(arguments));
      };
    }
  });
}

function patchAddListenerFns (obj, fnNames, options) {
  options = options || {};
  options.skipEnqueue = options.skipEnqueue || false;

  fnNames.forEach(function (fnName) {
    var delegate = obj[fnName];

    obj[fnName] = function (name, fn) {
      var boundFn = arguments[1] = global.zone.bind(arguments[1], options.skipEnqueue);
      var map = this[keys.common.bindingMap] = this[keys.common.bindingMap] || new BindingMap();
      map.add(name, fn, boundFn);
...
module.exports = {
  patchAddListenerFns: patchAddListenerFns,
