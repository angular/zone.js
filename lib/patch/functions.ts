...
export function patchSetClearFunction(window, Zone, fnArgs) {
  for(var i = 0; i < fnArgs.length; i++) {
    var fnArg = fnArgs[i];
    var setName = fnArg[0];
    var clearName = fnArg[1];
    var repeating = fnArg[2];
    var isRaf = fnArg[3];
    (function(setName, clearName, repeating, isRaf) {
    ...
    })(fnArg[0], fnArg[1], fnArg[2], fnArg[3]);
  }
};
